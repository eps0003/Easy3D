...
shared class CMatrix
{
	...
	CMatrix(float width, float height, float depth)
	{
		Orthographic(width, height, depth);
	}

	CMatrix(float fovRadians, float aspectRatio, float nearPlane, float farPlane)
	{
		Perspective(fovRadians, aspectRatio, nearPlane, farPlane);
	}
	...
	CMatrix@ Orthographic(float width, float height, float depth)
	{
		Matrix::MakeOrtho(matrix, width, height, depth);
		return this;
	}

	CMatrix@ Perspective(float fovRadians, float aspectRatio, float nearPlane, float farPlane)
	{
		Matrix::MakePerspective(matrix, fovRadians, aspectRatio, nearPlane, farPlane);
		return this;
	}
